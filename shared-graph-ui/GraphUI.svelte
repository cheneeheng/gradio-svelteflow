<script lang="ts">
  ...
  import type { Attribute, CollapsibleEdgeData, CustomEdge, CustomNode, Graph } from "./types/schemas";
  ...
  export let value: Graph = { nodes: [], edges: [], viewport: { x: 0, y: 0, zoom: 1 }};
  ...
  function handleLayout() {
    ...
    value.nodes = [...layoutedNodes];
    value.edges = [...layoutedEdges];
  }
  ...
  function handleConnect(connection: Connection) {
    ...
    let currentEdges = value.edges.filter(
      (e) =>
    ...
    value.edges = [...value.edges, newEdge];
  }
  ...
  function handleDelete({
    ...
    clickedEdges.set([]);
    searchedNodes.set([]);
    value.nodes = value.nodes.filter((n) => !deletedNodes.some((dn) => dn.id === n.id));
    value.edges = value.edges.filter((e) => !deletedEdges.some((de) => de.id === e.id));
  }
  ...
  function handleAddNode() {
    ...
    const x = (cx - value.viewport.x) / value.viewport.zoom;
    const y = (cy - value.viewport.y) / value.viewport.zoom;
    const id = uuidv4();
    ...
    value.nodes = [...value.nodes, newNode];
  }



  function handleNodeCollapse(nodeId: string, isCollapsed: boolean) {
    value.edges = value.edges.map((edge) => {
    ...
    if (clickTimer) {
      ...
      clickTimer = setTimeout(() => {
        ...
        if (interactive && e.detail.event instanceof MouseEvent) {
          ...
          const connectedEdges = value.edges.filter(
            (edge) =>
  ...
  function handleSaveNode(event: CustomEvent<CustomNode>) {
    ...
    const isNameDuplicate = value.nodes.some(
      (n) =>
        n.id !== updatedNode.id &&
    ...
    value.edges = value.edges.filter((edge) => {
      if (
  ...
  function handleSearchInternal() {
    ...
    const matchingNodes = value.nodes.filter((node) =>
  ...
  function handleKeydown(event: KeyboardEvent) {
    if (event.key === "Escape") {
      value.nodes = value.nodes.map((n) => ({ ...n, selected: false }));
      value.edges = value.edges.map((e) => ({ ...e, selected: false }));
      clickedNodes.set([]);
      clickedEdges.set([]);
      searchedNodes.set([]);
      searchQuery = "";
